...
| K1 concurrent burst (stress shape) | 50 concurrent 1:1s hitting the same remote member: **zero loss** (50/50 into transcript), no crash, all reply | **RESOLVED (pre-launch).** The owner now acks EVERY coalesced correlationId at turn end with the same outcome (coordinator `handleWake` batches per conversationId), so no waiter hangs to the sync-wait timeout. Unit-proven: coordinator.test.ts "coalesced wakes on ONE session all get a turn-complete (K1)". |
...
| C5 (host-crash auto re-host) | **PASS — stable.** | **RESOLVED (pre-launch).** Re-host + survivor election is green; the production PeerDialer (address-book redial + roster re-entry auth) closes the transport re-form seam that was `NO_PEER_DIALER`. Verified **6 consecutive unattended federation runs, 0 FAIL** (the production-readiness bar). Removed from the exemption list. |

## Pre-launch security & robustness hardening (this round)

| Area | Before | Now |
|---|---|---|
| **D10 device-key identity binding** | invite minted `identity:''` → `getSessionIdentity` null → host anti-spoof gate inert; J1/J2/J4 SKIP | federation-node WS sessions prove their portable identity via a device-key **challenge–response** (`http/websocket.ts`); `getSessionIdentity` returns the proven id; the host asserts `fromNode === session id` and drops spoofed-origin frames (incl. `turn-complete`/`stream-frames` origin checks). Bearer-only office sessions are viewers that **cannot speak federation frames**. Unit-proven: `tests/unit/http/websocket-federation-auth.test.ts` (valid proof binds identity; wrong-nonce replay rejected; bearer viewer refused; roster re-entry admits only an already-admitted identity) + `anti-spoof.test.ts`. **J1/J2 are now proven at the WS/manager layer** (the HTTP cluster harness cannot forge WS frames, so scenarios J1/J2 stay SKIP by surface, not by gap). |
| **Narrow scope (permission overlay)** | `issueOfficeCredential` rejected any non-default scope (`OFFICE_SCOPE_NOT_SUPPORTED`) → J4 dead | narrow scope is issuable end-to-end: proven identity at join → overlay persisted on the member row → authority scope-gate enforces it against the authenticated node; HTTP read routes project board **and artifacts** per-member. Unit-proven: `office-credential.test.ts`, `team-read.test.ts` (detail + artifacts projection). **J4 enabled** (cluster J4 stays SKIP only because the harness can't drive a device-key WS join). |
| **Protocol version negotiation** | `negotiateVersion` defined, never called | wired into the join handshake: a PRESENT-and-incompatible `pv` is rejected `VERSION_INCOMPATIBLE`; the grant echoes the negotiated pv + effective caps; absent-pv peers stay lenient (security is the device-key proof, not the version gate). Unit-proven: coordinator.test.ts version tests. |
| **Bounded mailbox (M-4)** | `mailboxBuffers` unbounded | per-session cap (128); overflow sheds the oldest with a warning. Unit-proven: message-bus.test.ts "caps the per-session mailbox (M-4)". |
| **Partition write gate** | `paused` was a UI signal only | shadow-office writes are refused with a calm retryable "the shared board is syncing" error while the office is paused (no divergent optimistic rows). |
| **One-click join** | invite link required manual paste; a browser click landed on the remote-control login page | `halo://join` protocol handler + a public invite landing page (no token echoed server-side) → "Open in Halo" auto-opens the join dialog pre-filled; cold-start deep links buffered and consumed on renderer start. |
