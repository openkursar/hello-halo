...
| Change what a NON-OWNER caller may make a digital human do (IM guest / teammate) | `src/shared/apps/capability-policy.ts` (vocabulary) + `src/main/apps/runtime/capability-policy.ts` (enforcement) | Both call sites in `apps/runtime/app-chat.ts`; the UI is `components/capability/CapabilityPolicyFields.tsx`, used by BOTH settings screens. Never fork the two scenarios. |
