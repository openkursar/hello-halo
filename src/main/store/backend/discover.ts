...
const serverLayout = createPageDocumentResolver(
  'store/discover',
  'fetchPageLayout',
  raw => validateLayout(raw as DiscoverLayout),
)
...
export async function getDiscoverLayout(): Promise<DiscoverLayout> {
  return (await serverLayout.get()) ?? BUILTIN_DISCOVER_LAYOUT
}
