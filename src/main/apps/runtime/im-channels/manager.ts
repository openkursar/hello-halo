...
export class ImChannelManager {
  ...
  getAllStatuses(): ImChannelInstanceStatus[] {
    return this.currentConfigs.map(cfg => this.buildStatus(cfg))
  }
  ...
  getInstanceStatus(instanceId: string): ImChannelInstanceStatus | null {
    ...
    if (!cfg) return null
    return this.buildStatus(cfg)
  }

  private buildStatus(cfg: ImChannelInstanceConfig): ImChannelInstanceStatus {
    const connected = this.instances.get(cfg.id)?.isConnected() ?? false
    return {
      id: cfg.id,
      type: cfg.type,
      enabled: cfg.enabled,
