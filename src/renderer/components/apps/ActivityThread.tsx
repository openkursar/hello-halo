...
import { RunsSummaryBand } from './RunsSummaryBand'
    ...
    <div>
      <main className="min-w-0">
        ...
        <section className="mb-7" aria-label={t('Needs you')}><h2 className="mb-4 text-base font-medium">{t('Needs you')} <span className="ml-2 text-sm text-muted-foreground">{(state?.pendingDecisionCount ?? active.length) + (state?.blocked ? 1 : 0)}</span></h2>
          ...
          {!active.length && !state?.blocked && !failed && !initialLoading && <p className="rounded-xl border border-dashed border-border p-5 text-sm text-muted-foreground">{t('Nothing is waiting for you.')}</p>}
