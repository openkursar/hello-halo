...
export function TaskSidebar({ teamId, tasks, selectedId, isOwner, onSelect, onNew, query, onQuery: setQuery, status, onStatus: setStatus }: {
  query: string; onQuery: (value: string) => void; status: string; onStatus: (value: string) => void
  teamId: string; tasks: TeamConversation[]; selectedId: string | null; isOwner: boolean
  ...
  const labels: Record<TaskGroup, string> = {
    ...
    other: t('Other team tasks'), reception: t('Reception'), automatic: t('Automated tasks'),
  }
  const search = query.trim().toLocaleLowerCase()
  const matches = tasks.filter(task => (task.waitingUser || status === 'all' || (status === 'completed' ? task.completed : !task.completed)) && (task.label || t('New task')).toLocaleLowerCase().includes(search))
    ...
    <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-4">
      ...
      {taskGroups.map(group => {
        ...
        return <section key={group} className="mb-3">
          {group === 'attention' ? <h3 className="flex items-center gap-1.5 px-2 py-2 text-xs font-medium text-halo-warning"><AlertCircle size={13} /><span className="flex-1">{labels[group]}</span><span className="tabular-nums">{grouped.length}</span></h3> : <button aria-expanded={open} aria-controls={`${listId}-${group}`} onClick={() => setGroup(teamId, group, !open)} className="flex w-full items-center gap-1.5 rounded-lg px-2 py-2 text-left text-xs text-muted-foreground hover:bg-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
          ...
          <div id={`${listId}-${group}`} hidden={!open} className="space-y-1">{open && grouped.map(task => <button key={task.epochId} onClick={() => onSelect(task.epochId)} title={task.label || t('New task')} aria-current={selectedId === task.epochId ? 'page' : undefined}
            ...
            <span className="mt-1.5 flex items-center gap-1.5 text-[11px] text-muted-foreground">
              ...
              <span className={`min-w-0 flex-1 truncate ${task.waitingUser ? 'text-halo-warning' : ''}`}>{task.waitingUser ? t('Waiting for your decision') : task.completed ? t('Completed') : task.active ? t('Working') : t('Ready')}</span>
